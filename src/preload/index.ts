...
const api: ElectronAPI = {
  app: {
    ...
    refreshStatus: () => invoke(IPC.APP_REFRESH_STATUS),
    getLogPath: () => invoke(IPC.APP_GET_LOG_PATH),
    openLogFolder: () => invoke(IPC.APP_OPEN_LOG_FOLDER),
    pickDirectory: () => invoke(IPC.APP_PICK_DIRECTORY),
  ...
  session: {
    ...
    inputRaw: (sessionId, data) => invoke(IPC.SESSION_INPUT_RAW, sessionId, data),
  ...
  skills: {
    list: (args) => invoke(IPC.SKILLS_LIST, args),
  },
